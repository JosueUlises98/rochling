...
public class UserConfiguration implements Configuration {
    ...
    public OpcUaClient createUserOpcUaClient() throws OpcUaConfigurationException {
        try {
            ...
            UserConfigFile configFile = new UserConfigFile(
                    new ObjectMapper(),
                    new Properties(),
                    new org.kopingenieria.config.opcua.user.UserConfiguration()
            );
    ...
    private void configurarSuscripciones(OpcUaClient client,
                                         org.kopingenieria.config.opcua.user.UserConfiguration userConfig) throws Exception {
        List<UaSubscription> subscriptions = new ArrayList<>();
        mapSubscriptions = new HashMap<>();
        mapSubscriptions.put(client, subscriptions);

        for (org.kopingenieria.config.opcua.user.UserConfiguration.Subscription subscriptionConfig :
    ...
    private UaSubscription configurarSuscripcion(OpcUaClient client, org.kopingenieria.config.opcua.user.UserConfiguration.Subscription subscriptionConfig) throws Exception {
