...
import org.kopingenieria.application.validators.contract.bydefault.DefaultSessionValidator;
...
public class DefaultSessionValidatorImpl implements DefaultSessionValidator {
    ...
    @Autowired
    private DefaultAuthenticationValidatorImpl authValidator;
    ...
    @Override
    public boolean validateSessionSecurityPolicy(UaClient client, DefaultSessionConfiguration user) {
        try {
    ...
    @Override
    public boolean validateSessionSecurityMode(UaClient client, DefaultSessionConfiguration user) {
        try {
            EndpointDescription endpoint = client.getConfig().getEndpoint();
    ...
    @Override
    public boolean validateSessionCertificate(UaClient client, DefaultSessionConfiguration user) {
        try {
            ...
            if (!validateDefaultSpecificCertificate(certificate, user)) {
                return false;
            }
    ...
    private boolean validateDefaultSpecificCertificate(X509Certificate certificate,
                                                    DefaultSessionConfiguration user) throws CertificateException, IOException {
